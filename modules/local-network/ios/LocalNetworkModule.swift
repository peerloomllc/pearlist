...
// or a direct connection to a same-subnet address. PearList's transport is
...
// browsing a throwaway Bonjour service (`_pearlistlan._tcp`, declared in
...
public class LocalNetworkModule: Module {
  ...
  private let queue = DispatchQueue(label: "com.pearlist.localnetwork")
  private let serviceType = "_pearlistlan._tcp"
  ...
  private func startProbe() {
    queue.async { [weak self] in
      ...
      if let listener = try? NWListener(using: .tcp) {
        listener.service = NWListener.Service(name: "PearList", type: self.serviceType)
