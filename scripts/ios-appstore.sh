...
# ── On-device AI worker bundle ──────────────────────────────────────────────
# HARD GATE, because this failure is otherwise invisible: the Xcode build and the
# App Store upload BOTH succeed, and the app only breaks in the user's hand when
# they enable Local AI ("Failed to load mobile worker bundle").
#
# @qvac/sdk/dist/worker.mobile.bundle.js is generated by `expo prebuild`, which
# only ever runs on the Linux box, and every sync to this Mac excludes
# node_modules - so this file arrives only because release.sh / ios-dev-install.sh
# copy it explicitly (scripts/sync-qvac-worker.sh). If it is not here, the archive
# would ship an app whose on-device AI cannot start. Refuse to build instead.
QVAC_WORKER="$REPO_ROOT/node_modules/@qvac/sdk/dist/worker.mobile.bundle.js"
if [ ! -s "$QVAC_WORKER" ]; then
  echo "Error: refusing to archive - the QVAC mobile worker bundle is missing on this host." >&2
  echo "  Expected: $QVAC_WORKER" >&2
  echo "  It is a prebuild output, not an npm one, so 'npm install' here cannot fix it." >&2
  echo "  Sync it from the machine that ran expo prebuild:" >&2
  echo "    ./scripts/sync-qvac-worker.sh <this-host> <repo-path>" >&2
  echo "  Building without it produces an app whose on-device AI fails to start." >&2
  exit 1
fi
echo "QVAC worker bundle: present ($(du -h "$QVAC_WORKER" | cut -f1))"
