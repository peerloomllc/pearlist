...
if [ "${SKIP_SYNC:-0}" != "1" ]; then
  ...
  # One file in node_modules is a PREBUILD output, not an npm one: @qvac/sdk's
  # mobile worker bundle, generated by `expo prebuild` on the Linux box. npm
  # install cannot produce it, so it has to be copied after that install (which
  # may have replaced the package dir). Without it the app builds fine and then
  # fails at runtime with "Failed to load mobile worker bundle" - no on-device AI.
  step "sync QVAC worker bundle to $MAC_MINI"
  "$REPO_ROOT/scripts/sync-qvac-worker.sh" "$MAC_MINI" "$MAC_REPO_PATH"
fi
