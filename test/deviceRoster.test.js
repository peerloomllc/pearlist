...
// THE IMPORTANT ONE. `device:remove` hides a row. There is no writer revocation
// anywhere in device-link, so a removed phone keeps the mnemonic, keeps the
// spaces and can still write to them. Telling someone their lost phone has been
// cut off when it has not is the worst thing to be wrong about on this screen.
test('the removal confirm does not claim to lock the other phone out', () => {
  const src = app()
  ...
  assert.match(body, /does NOT lock that phone out/i, 'and must say what it does not do')
  assert.match(body, /recovery phrase/i, 'naming why: it still holds the phrase')

  // Words that would promise revocation.
  for (const claim of [/\brevoke/i, /\bcut off\b(?!.*not)/i, /\bblocked?\b/i, /can no longer/i]) {
    assert.doesNotMatch(body, claim, `removal copy must not imply revocation: ${claim}`)
  }
})

test('listMethods no longer claims device:remove blocks the writer', () => {
  // The comment used to say "Hides a device from the roster and blocks its
  // writer". Only the first half was true, and comments are where the next
  // person forms their mental model.
  const raw = read('src/listMethods.js')
  assert.doesNotMatch(raw, /blocks its writer/, 'that claim was false')
  assert.match(raw, /There is no removeWriter in device-link/, 'say so explicitly instead')
})
